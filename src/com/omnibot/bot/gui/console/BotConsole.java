package com.omnibot.bot.gui.console;
...
public class BotConsole extends JList<Object> {
	...
	public BotConsole() {
		...
		scroller = new JScrollPane(this, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS, JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
	...
	private static class Renderer implements ListCellRenderer<Object> {
		...
		private Border SELECTED_BORDER = BorderFactory.createLineBorder(Color.GREEN, 1);
		...
		public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean isSelected, boolean cellHasFocus) {
			...
			if (ce.record.getLevel() == Level.WARNING) {
				jta.setForeground(Color.RED.darker());
			}
			else if (ce.record.getLevel() == Level.CONFIG) {
				jta.setForeground(Color.GREEN.darker());
			}
			else if (ce.record.getLevel() == Level.SEVERE) {
